...
$active_managed = (bool) $active_meta; // true = gerida pelo Gigantic, false = externa

if ( $active_meta ) {
    // Tabela Gigantic — schema vem do nosso meta
    $active_schema = json_decode( $active_meta->schema_json, true ) ?: [];
} elseif ( $gig_table ) {
    // Tabela externa — lê schema via DESCRIBE
    global $wpdb;
    $active_schema = gig_crud_get_external_columns( $wpdb->prefix . $gig_table );
    // Simula um $active_meta mínimo para o template não ficar em branco
    $active_meta              = new stdClass();
    $active_meta->table_key   = $gig_table;
    $active_meta->table_label = gig_crud_humanize( $gig_table );
...
$all_db_tables = gig_crud_get_all_db_tables();
...
/**
 * Sidebar — lista tabelas Gigantic + externas agrupadas por origem.
...
$source_order = [ 'gigantic', 'jetengine', 'woocommerce', 'gravityforms', 'acf', 'buddypress', 'learndash', 'rankmath', 'yoast', 'wpcore', 'other' ];
 ...
 * Tabelas Gigantic têm ações completas. Externas têm apenas "Ver" e "Importar".
 */
...
$source_order = [ 'gigantic', 'jetengine', 'woocommerce', 'gravityforms', 'acf', 'buddypress', 'learndash', 'rankmath', 'yoast', 'other', 'wpcore' ];
...
$source_badge_class = [
    'gigantic'    => 'gig-badge-green',
    'jetengine'   => 'gig-badge-orange',
    'woocommerce' => 'gig-badge-purple',
    'wpcore'      => 'gig-badge-blue',
    'other'       => 'gig-badge-grey',
];
?>