 ...
 */
function gig_crud_render_field_input( array $field, string $input_name, $value = '', string $form_id = '' ): string {
    ...
    if ( $type === 'IMAGE' ) {
        return gig_crud_render_image_input( $input_name, $value, $form_id, $required );
    }

    if ( $type === 'TEXT' || $type === 'LONGTEXT' ) {
        return '<textarea ' . $name_attr . ' ' . $form_attr . ' class="gig-input gig-input-sm" rows="2" ' . $required . '>' . esc_textarea( $value ) . '</textarea>';
    }
    ...
    if ( $type === 'INT' || $type === 'MEDIUMINT' || $type === 'BIGINT' || $type === 'DECIMAL' || $type === 'FLOAT' ) {
        return '<input type="number" ' . $name_attr . ' ' . $form_attr . ' class="gig-input gig-input-sm" value="' . $val . '" ' . $required . '>';
    }

    if ( $type === 'DATE' ) {
        return '<input type="date" ' . $name_attr . ' ' . $form_attr . ' class="gig-input gig-input-sm" value="' . $val . '" ' . $required . '>';
    }

    if ( $type === 'DATETIME' ) {
        return '<input type="datetime-local" ' . $name_attr . ' ' . $form_attr . ' class="gig-input gig-input-sm" value="' . esc_attr( str_replace( ' ', 'T', $value ) ) . '" ' . $required . '>';
    }

    // Default — VARCHAR e outros
    return '<input type="text" ' . $name_attr . ' ' . $form_attr . ' class="gig-input gig-input-sm" value="' . $val . '" ' . $required . '>';
}

/**
 * Renderiza o campo de imagem com preview e botão do Media Uploader.
 */
function gig_crud_render_image_input( string $input_name, $value, string $form_id, string $required ): string {
    $form_attr  = $form_id ? 'form="' . esc_attr( $form_id ) . '"' : '';
    ...
    ob_start();
    ?>
    <div class="gig-media-field">
        <input type="hidden"
               name="<?php echo esc_attr( $input_name ); ?>"
               <?php echo $form_attr; ?>
               class="gig-media-input"
               value="<?php echo esc_attr( $value ); ?>"
               <?php echo $required; ?>>

        <div class="gig-media-preview"><?php echo $preview_html; ?></div>

        <div style="display:flex;flex-direction:column;gap:4px;">
            <button type="button" class="gig-btn gig-btn-ghost gig-media-btn-open" style="padding:5px 10px;font-size:10px;">
            ...
            <button type="button" class="gig-btn gig-btn-danger gig-media-btn-clear"
